...
def generate_review(name, aff_link):
    try:
        API_URL = "https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.1"
        headers = {"Authorization": f"Bearer {os.getenv('HF_TOKEN', '')}"}
        prompt = f"Write a short review for {name} and include the link {aff_link}"
...
def post_blog(title, content):
    with sync_playwright() as p:
        ...
        pg.fill("textarea#content", content)
        pg.click("input#publish")
        time.sleep(2)
        b.close()
...
def tweet(content):
    ...
    with sync_playwright() as p:
        ...
        pg.click("div[data-testid='tweetButton']")
        time.sleep(2)
        b.close()
...
def main():
    ...
    for name, link in fetch_products():
        ...
        review = generate_review(name, aff_link)
        if WP_COOKIES:
            post_blog(name, review)
        if TW_COOKIES:
            tweet(f"Check out {name}: {aff_link}")
