...
HIVE_ACC = os.getenv("HIVE_ACCOUNT")
HIVE_KEY = os.getenv("HIVE_ACTIVE_KEY")
...
def swap_to_eth(amount_hive):
    ...
    op = operations.Transfer(
        {"from": HIVE_ACC, "to": addr, "amount": f"{amount_hive:.3f} HIVE", "memo": ""}
    )
...
def main():
    hive = Hive()
    acc = Account(HIVE_ACC, hive_instance=hive)
    # balance is an Amount object – convert to float directly
